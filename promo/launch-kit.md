...
- 安装包：约 1.5 MB
- 核心工作流：快捷键进入绘制，`X` 穿透，`W` 白板，`Esc` 退出
...
Marker 是一款轻量级开源屏幕标注工具，面向演示、教学、会议与录屏。它从快捷键启动，可在任意应用上绘制，并提供穿透模式——标注保持可见的同时继续操作下层应用。

## 一句话简介

- 轻量开源的屏幕标注工具，支持穿透模式。
- 在桌面任意位置绘制，标注保持可见的同时点击穿透到下层应用。
- 为想要快速标注、不想套用重型演示套件的人准备的键盘优先小工具。
- 1.5 MB 的 Tauri 应用，集屏幕标注、白板模式与穿透绘制于一体。

## 短描述

Marker 是一个小巧的开源桌面屏幕标注应用。按下快捷键，在任意应用上绘制；需要操作下层屏幕时切换穿透模式，讲解需要补标注时随时切回绘制。支持 10 种标注工具、白板模式、全键盘快捷键，提供 Windows 与 macOS 构建。

## 长描述

Marker 是基于 Tauri v2、Rust、Vue 与 Canvas 构建的轻量级屏幕标注工具，专为需要现场讲解的人设计：教师、培训师、会议主持、演示者、教程作者与开发者。

应用静默运行在系统托盘。按 `Alt+G` 进入标注模式，在任意桌面应用上绘制，用数字键切换工具，按 `X` 或 `Alt+M` 在绘制与穿透模式间切换。穿透模式下标注保持可见，而鼠标点击、滚动与输入直达下层应用。
...
Marker 是一个微小的开源桌面应用，用于在演示、教学、会议与录屏时在屏幕上绘制。标注保持可见的同时鼠标事件穿透到下层应用，并可随时切回绘制。

### 作者评论

大家好 Product Hunt！我做 Marker 是因为想要一个不像重型演示套件的快速屏幕标注工具。

它运行在托盘，从快捷键唤醒，可以在任意桌面应用上绘制。穿透模式让标注保持可见的同时继续操作下层应用，并且无需退出会话即可切回绘制。
...
它面向演示、教学、会议与录屏：按下快捷键，在任意应用上绘制，从键盘切换工具。穿透模式让标注保持可见而鼠标事件穿透到下层应用。这让它在 Epic Pen / ZoomIt 类工作流中可用，同时保持小巧、本地优先、开源。
...
在任意桌面应用上绘制 / 标注保持可见 / 点击穿透到下层应用 / 随时切回绘制
...
它是一个带穿透模式、白板模式和全键盘控制的轻量级屏幕标注工具。
...
Marker 专注于轻量标注、白板模式与穿透绘制。ZoomIt 在缩放上依旧出色；需要缩放时 Marker 与系统放大器搭配良好。

### Marker vs Epic Pen

Marker 提供开源、本地优先的替代方案：安装包小、键盘优先控制，以及标注持久可见的穿透模式。
...
Marker 的用法很简单：按 `Alt+G` 直接在桌面上画；按 `X` 后标注仍然可见，但鼠标可以继续点下面的软件；按 `W` 变白板；按 `Esc` 退出。
...
我是 Marker 的开发者，一款面向 Windows 与 macOS 的轻量级开源屏幕标注应用。它为现场讲解而生：按下快捷键在任意应用上绘制，再用穿透模式保持标注可见的同时继续操作下层应用。
...
ZoomIt 很出色，尤其缩放。Marker 专注跨平台屏幕绘制、白板模式，以及标注持久可见的穿透标注。
