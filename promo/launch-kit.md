...
Marker v2.2.0 is out with an official portable Windows build.
...
Marker v2.2.0 已发布，这次新增了官方 Windows 绿色免安装版。
...
Marker is a lightweight open-source screen annotation tool for demos, teaching, meetings, and recordings. It starts from a hotkey, draws over any app, and includes click-through mode so annotations stay visible while you interact with the apps underneath.
...
Marker is a small, open-source desktop app for screen annotation. Press a hotkey, draw over any app, switch to click-through mode when you need to operate the screen below, and jump back to drawing whenever the explanation needs another mark. It supports 10 annotation tools, whiteboard mode, keyboard shortcuts, and official builds for Windows and macOS.

## Long Description

Marker is a lightweight screen annotation tool built with Tauri v2, Rust, Vue, and Canvas. It is designed for people who explain things live: teachers, trainers, meeting hosts, demo presenters, tutorial makers, and developers.
...
Marker also includes a whiteboard mode, undo/redo, text, arrows, shapes, eraser modes, angle snapping, copy-to-clipboard, expandable toolbar panel, and content preservation between sessions.
...
Marker is a tiny open-source desktop app for drawing over your screen during demos, teaching, meetings, and recordings. Keep annotations visible while mouse events pass to the apps underneath, then switch back to drawing instantly.

### Maker Comment

Hi Product Hunt! I built Marker because I wanted a fast screen annotation tool that did not feel like a heavy presentation suite.
...
- 10 annotation tools, including a laser pointer and numbered stamps
- click-through mode
- whiteboard mode
...
Show HN: Marker – open-source screen annotation with click-through

Post:

Hi HN, I built Marker, a lightweight open-source desktop app for screen annotation.
...
Marker：约 1.5 MB 的开源屏幕标注工具，支持穿透模式
...
Marker is a tiny open-source screen annotation app.
...
I built Marker because I wanted screen annotation to feel instant:
...
Teachers, demo presenters, and tutorial makers: I would love feedback on Marker.
...
2. Why Marker is keyboard-first.
...
If you teach, record tutorials, run demos, or use ZoomIt / Epic Pen-style tools, I would love your feedback on Marker.
...
Shot 2: Marker overlay appears; draw arrow and highlight.
Text: "Draw anywhere, instantly."
End card: "Marker - open-source screen annotation"
...
Marker focuses on lightweight annotation, whiteboard mode, and click-through drawing. ZoomIt remains excellent for zooming; Marker pairs well with system magnifiers when zoom is needed.

### Marker vs Epic Pen

Marker provides an open-source, local-first alternative with a small installer, keyboard-first controls, and click-through mode for persistent annotations while interacting with apps below.

### Marker vs meeting whiteboards

Meeting whiteboards are tied to a meeting app. Marker works over any desktop app, needs no account, and keeps drawings local (no cloud upload).
...
Marker 的用法很简单：按 `Ctrl+Shift+D` 直接在桌面上画；按 `X` 后标注仍然可见，但鼠标可以继续点下面的软件；按 `W` 变白板；按 `Esc` 退出。
...
如果你的需求是「演示过程中直接在任意软件上画重点」，而不是截图后再编辑，我更建议用专门的桌面标注层。Marker 是我做的免费开源方案：约 1.5 MB，支持 Windows / macOS，快捷键进入后可用画笔、荧光笔、激光笔、箭头、形状、文字和序号；切到穿透模式后，标注保持可见，同时还能继续操作下层软件。
...
Marker is a small MIT-licensed app for drawing over any desktop application during demos, teaching, meetings, and recordings.
...
I made Marker for the moments when you need to circle a control, highlight a sentence, or sketch a quick diagram without leaving the app you are teaching from.
...
I am the developer of Marker, a lightweight open-source screen annotation app for Windows and macOS. It is built for live explanation: press a hotkey to draw over any app, then use click-through mode to keep marks visible while continuing to operate the app underneath.
...
Marker runs locally and does not require an account, telemetry, or cloud upload. The source is available under MIT, and official downloads are linked from https://marker.cn/.

### Why not ZoomIt?

ZoomIt is excellent, especially for zooming. Marker focuses on cross-platform screen drawing, whiteboard mode, and click-through annotations that stay visible while you keep using the app below.

### Why not Epic Pen?

Epic Pen is a polished commercial option. Marker is a small, open-source, local-first alternative with keyboard-first controls. Which is better depends on whether you value the commercial feature set or an inspectable open-source tool.

### Linux?

Marker currently supports Windows and macOS. Do not promise a Linux date; link interested users to the issue tracker.

## UTM Convention

Use the website as the public link when the channel allows it. Add UTMs only where they do not look noisy:

`https://marker.cn/?utm_source={{channel}}&utm_medium={{format}}&utm_campaign=evergreen_2026`
