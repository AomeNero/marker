...
**Goal:** Make the Windows system tray icon follow `SystemUsesLightTheme` (taskbar / overflow flyout), including live updates while Marker is running, independent of `general.theme`.
...
1. Dark taskbar / overflow flyout → light Marker tray glyph (`icon-light.png`) is visible on the dark mica flyout.
2. Light taskbar / flyout → dark glyph (`icon.png`) is visible.
3. With Marker running, flip Windows **Settings → Personalization → Colors → Choose your mode** (or “Windows mode” / taskbar-related mode) so the flyout light/dark changes → tray updates **without** restarting Marker.
4. In Marker settings, switch Appearance Dark ↔ Light ↔ System → **tray stays** on taskbar signal; settings window chrome/icon still follows app theme.
