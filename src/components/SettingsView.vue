...
/** Help tab mirrors the user's actual shortcuts (falls back to defaults). */
...
async function openOnlineHelp() {
  try {
    await invoke('open_url', { url: 'https://marker.cn/help.html' })
  } catch (error) {
  ...
  <div class="flex h-full w-full font-text select-none overflow-hidden settings-text-label">
    ...
    <div class="relative z-10 w-41 shrink-0 settings-sidebar flex flex-col ui-divider-v">
      <div class="flex items-center gap-2.5 px-4 pt-5 pb-5">
        <svg class="w-7 h-7 shrink-0" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
          <path
            fill="#13227a"
        ...
        <span class="font-semibold settings-text-brand tracking-wide leading-tight">Marker</span>
      </div>
